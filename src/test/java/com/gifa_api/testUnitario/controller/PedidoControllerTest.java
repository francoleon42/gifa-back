...
public class PedidoControllerTest {
    ...
    void generarPedidoManual_devuelveStatusOk() {
        doNothing().when(pedidoService).crearPedidoManual(new CrearPedidoDTO());
    ...
    @Test
    void verPedidosRechazadosYpendientesYinsuficientes_devuelveStatusOkYlosPedidos() {
        ...
        ,PedidoResponseDTO.builder().idPedido(2).estadoPedido("PENDIENTE").build(),
                PedidoResponseDTO.builder().idPedido(3).estadoPedido("PRESUPUESTO_INSUFICIENTE").build());

        when(pedidoService.obtenerPedidosRechazadosYpendientesYpresupuestoInsuficiente()).thenReturn(pedidosRechazadosYpendientes);

        assertEquals(HttpStatus.OK, pedidoController.verPedidosRechazadosYpendientesYpresupuestoInsuficiente().getStatusCode());
        assertEquals(pedidosRechazadosYpendientes, pedidoController.verPedidosRechazadosYpendientesYpresupuestoInsuficiente().getBody());
    }
