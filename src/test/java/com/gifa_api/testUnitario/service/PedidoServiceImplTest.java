...
class PedidoServiceImplTest {
    ...
    @Test
    void obtenerPedidosRechazadosYpendientesYpresupuestoInsuficienteSinProveedor() {
        List<PedidoResponseDTO> pedidosResponseDTO = List.of(
                PedidoResponseDTO.builder().estadoPedido("RECHAZADO").build(),
                PedidoResponseDTO.builder().estadoPedido("PENDIENTE").build(),
                PedidoResponseDTO.builder().estadoPedido("PRESUPUESTO_INSUFICIENTE").build(),
                PedidoResponseDTO.builder().estadoPedido("SIN_PROVEEDOR").build()
        );

        when(pedidosMapper.mapToPedidoDTO(pedidoRepository.findPedidosByCuatroEstados(
                EstadoPedido.PENDIENTE,
                EstadoPedido.RECHAZADO,
                EstadoPedido.PRESUPUESTO_INSUFICIENTE,
                EstadoPedido.SIN_PROVEEDOR))).thenReturn(pedidosResponseDTO);

        List<PedidoResponseDTO> result = pedidoService.obtenerPedidosRechazadosYpendientesYpresupuestoInsuficienteSinProveedor();

        assertEquals(4, result.size());

        verify(pedidosMapper, times(1)).mapToPedidoDTO(pedidoRepository.findPedidosByCuatroEstados(
                EstadoPedido.PENDIENTE,
                EstadoPedido.RECHAZADO,
                EstadoPedido.PRESUPUESTO_INSUFICIENTE,
                EstadoPedido.SIN_PROVEEDOR));
    }
